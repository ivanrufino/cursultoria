...
<div class="easyui-tabs" fit="true" border="false" id="tt">
    ...
    <form id="form_grupoUsuarios" name="form_add" method="post" action="" style="overflow-y:visible;">
    	<input type="hidden" name="CODIGO" value="<?=$grupo->CODIGO?>" />
        <div id="tabs-1" style="height:300px; overflow-y:scroll;"> 
            <fieldset>
                <label>Nome<br/>
                	<input type="text" name="DESCRICAO" class="textfield-text" value="<?=$grupo->DESCRICAO?>" style="width:200px" />
                </label>
                <label style="margin-right:0px;"">E-mail<br/>
                	<input type="text" name="EMAIL" class="textfield-text" value="<?=$grupo->EMAIL?>" style="width:240px;"/>
                </label>
            </fieldset>
                            ...
                            <td style="padding:3px;">
                            	<select name="COD_USUARIO[]" class="getUsuarios" multiple='multiple' style="width:410px;">
                                	<?php foreach($usuarios as $usuario):
										$in = $this->users->inGrupo($grupo->CODIGO, $usuario->CODIGO);
									?>
                                   
                                		<option value="<?=$usuario->CODIGO?>" <?=($in != NULL)? 'selected="selected"': ''?> ><?=$usuario->NOME?></option>
                                    <?php endforeach;?>
                                </select>
                            </td>
