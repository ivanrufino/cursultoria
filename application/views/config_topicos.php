...
<!-- ./table_busca_div -->
    ...
    <?php foreach($lista_disciplinas as $disciplinas):?>   	
	
        <div class="accordion" id="lista_servicos" style="float:left; width:100%;">
            <div class="accordion-group">
            <div class="accordion-heading">
              <a class="accordion-toggle" data-toggle="collapse" data-parent="#lista_servicos" href="#cod_<?=limpaChars($disciplinas->CODIGO);?>">
                
                <?php 
				$lista_topicos = $this->cadastros->getTopicosByDisciplina($disciplinas->CODIGO);
				?>
                
                <?=$disciplinas->DESCRICAO;?> &nbsp;(<?=count($lista_topicos)?>)
              </a>
            </div>
           
            <div id="cod_<?=limpaChars($disciplinas->CODIGO);?>" class="accordion-body collapse">
                
            <?php if($lista_topicos == NULL){?>
            ...
            <script type="text/javascript">
				$('#all_<?=limpaChars($disciplinas->DESCRICAO);?>').toggle(
					
					function() {
						$('.table_<?=limpaChars($disciplinas->DESCRICAO);?> .ck_<?=limpaChars($disciplinas->DESCRICAO);?>').prop('checked', true);
						$(".ck_<?=limpaChars($disciplinas->DESCRICAO);?>").parent().parent('.td_line, .td_line_alter').addClass("td_line_selected");
					},
					function() {
						$('.table_<?=limpaChars($disciplinas->DESCRICAO);?> .ck_<?=limpaChars($disciplinas->DESCRICAO);?>').prop('checked', false);
	
						$(".ck_<?=limpaChars($disciplinas->DESCRICAO);?>").parent().parent('.td_line, .td_line_alter').removeClass("td_line_selected");
					}
				);
            </script>
            <form class="form_select" name="formulario" method="post" action="">
            <div id="users-contain" class="ui-widget">
                <table width="100%" id="users" class="table_<?=limpaChars($disciplinas->DESCRICAO);?> ui-widget ui-widget-content">
                    <thead>
                        <tr>
                            <th width="1%"><input type="checkbox" id="all_<?=limpaChars($disciplinas->DESCRICAO);?>" /></th>
                            <th width="20%">Nome</th>
<!--                            <th width="20%">Descricão</th>
                    ...
                    <?php 
					foreach($lista_topicos as $key =>$row):
                    ...
                    <label for="item" style="margin:0px;">
                      <td><input type="checkbox" id="item" class="ck_<?=limpaChars($disciplinas->DESCRICAO);?>" name="item[]" value="<?=$row['CODIGO']?>" /></td>
                        <td>
                            <a data-id="<?=$row['CODIGO']?>" rel="topicoDetalhe">
                                <b><?=$row['DESCRICAO']?></b>
                            </a>
                        </td>
<!--                        <td ><?=$row->DESCRICAO?></td>
